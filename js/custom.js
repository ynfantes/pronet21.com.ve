...
// Contact Form 
    $(".contactform").validate({
        submitHandler: function(form) {
            ...
            if ($(".contactform").attr("name") == 'cotiza') {
                accion  =  'email_cotiza'
                dataString += '&servicio=' + $("select#servicio").val() + '&celular=' + $("input#celular").val()
            } else {
                accion = 'email_contacto';
            }
            ga('send','event','contacto','enviar',accion);
            
            
            $.ajax({
                type: "POST",
                url: "includes/json.php?accion=" + accion,
                data: dataString,
                success: function() {
                    $('#contactmsg').remove();
                    $('.contactform').prepend("<div id='contactmsg' class='successmsg'>Menseje enviado con éxito! En breve lo estaremos contactando.</div>");
                    $('#contactmsg').delay(3000).fadeOut(500);
